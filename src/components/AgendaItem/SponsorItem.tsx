...
export default function SponsorItem({ talkInfo }: SponsorItemProps) {
  ...
  return (
    <article className="flex flex-col bg-[#DFF6FF] md:flex-row md:items-center min-h-14 p-2">
      <p className="w-1/4 min-w-auto font-bold md:text-center text-sm mt-1 md:text-base">
        {dayjs(startDate).format("hh:mm A")}
      </p>

      <div className="flex flex-wrap flex-row md:items-center w-full gap-3">
        {sponsors.map((sponsor) => (
          <img
            key={sponsor}
            src={`/assets/images/sponsors/${sponsor}.png`}
            alt={sponsor}
            className="w-18 md:w-20 object-contain"
          />
        ))}
